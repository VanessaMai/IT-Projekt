...
/**
 * Ein <code>ReportWriter</code>, der Reports mittels HTML formatiert. Das im Zielformat vorliegende Ergebnis wird in
 * der Variable <code>reportText</code> abgelegt und kann nach Aufruf der entsprechenden Prozessierungsmethode mit
...
public class HTMLReportWriter extends ReportWriter {
	...
	/**
	 * Diese Variable wird mit dem Ergebnis einer Umwandlung (vgl. <code>process...</code>-Methoden) belegt. Format:
	 * HTML-Text
	 */
	...
	public String getHeader() {
		...
		result.append("<!DOCTYPE html><html><head><title>Report</title><meta charset=\"UTF-8\"/></head><body>");
	...
	/**
	 * Prozessieren des übergebenen Reports und Ablage im Zielformat. Ein Auslesen des Ergebnisses kann später mittels
	 * <code>getReportText()</code> erfolgen.
	 * 
	 * @param r
	...
	public void process(AllEventsForRoomInPeriodOfTimeReport r) {
		...
		/*
		 * Nun werden Schritt für Schritt die einzelnen Bestandteile des Reports ausgelesen und in HTML-Form übersetzt.
		 */
		result.append("<H1>" + r.getTitle() + "</H1>");

		if (r.getHeaderData() != null) {
			result.append(paragraph2HTML(r.getHeaderData()));
		}
		...
		result.append("</table>");
		result.append(r.getCreated().toString());
		result.append(paragraph2HTML(r.getImprint()));
		/*
		 * Zum Schluss wird unser Arbeits-Buffer in einen String umgewandelt und der reportText-Variable zugewiesen.
		 * Dadurch wird es möglich, anschließend das Ergebnis mittels getReportText() auszulesen.
		 */
